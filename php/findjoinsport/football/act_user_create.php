<?php 
	...
	*/
	...
	$stmt = $conn->prepare("SELECT a.id , a.stadium_name , a.description , a.photo , a.date , a.time , a.location , a.number_join , a.status_id , u.user_id , u.name , u.photo_user FROM activity a INNER JOIN users u on a.user_id = u.user_id WHERE a.user_id = '$user_id' ORDER BY a.id DESC");
...
//SELECT a.id , a.stadium_name , a.date , a.time ,a.photo,u.user_id, u.name , u.photo_user 
	...
	$stmt->bind_result($id, $stadium_name, $description, $photo, $date, $time, $location, $number_join, $user_id, $name, $photo_user, $status_id);
