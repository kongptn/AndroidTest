<?php 
	...
	*/
	...
	$stmt = $conn->prepare("SELECT rq.req_id , a.id , a.stadium_name , a.date , a.time , a.user_id , s.status_id , s.status_name , u.user_id , u.name , u.photo_user 
	FROM request_joinact rq INNER JOIN activity a on rq.id = a.id INNER JOIN status s on rq.status_id = s.status_id INNER JOIN users u on rq.user_create = u.user_id WHERE rq.userid_join = '$user_id' ORDER BY rq.req_id DESC");
